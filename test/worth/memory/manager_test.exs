defmodule Worth.Memory.ManagerTest do
  ...
  describe "remember/2 and search/2" do
    test "stores and retrieves a fact globally" do
      {:ok, _entry} =
        Manager.remember("User prefers Elixir over Python",
      ...
      {:ok, %{entries: entries}} =
        Manager.search("programming language preference",
          workspace: "test-workspace"
        )
    ...
    test "tags entries with workspace metadata" do
      {:ok, entry} =
        Manager.remember("This project uses conventional commits",
          entry_type: "observation",
    ...
    test "returns empty results when memory is disabled" do
      {:ok, nil} =
        Manager.remember("test", enabled: false)

      {:ok, nil} =
        Manager.search("test", enabled: false)
    end
  end
  ...
  describe "recent/1" do
    test "returns recent entries via direct remember + query" do
      {:ok, _} =
        Manager.remember("Recent fact one",
          entry_type: "observation",
          workspace: "test"
        )

      {:ok, entries} = Manager.recent(workspace: "test", limit: 5)
      assert is_list(entries)
  ...
  describe "working memory" do
    test "push and read working memory entries" do
      {:ok, _} =
        Manager.working_push("Session note",
          workspace: "test-ws",
          importance: 0.7
        )

      {:ok, entries} = Manager.working_read(workspace: "test-ws")
    ...
    test "flush promotes high-importance entries to global store" do
      {:ok, _} =
        Manager.working_push("Important finding",
      ...
      {:ok, _} =
        Manager.working_push("Trivial note",
          workspace: "flush-test",
          importance: 0.2
        )

      {:ok, count} = Manager.working_flush(workspace: "flush-test")
      assert count >= 1
    end
  end
  ...
  describe "build_memory_context/2" do
    test "returns formatted context or nil when no results" do
      result =
        Manager.build_memory_context("nonexistent topic xyz",
          workspace: "ctx-test"
        )
