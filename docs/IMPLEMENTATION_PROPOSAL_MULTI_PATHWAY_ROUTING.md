...
| **Follow-up: full ACP discovery DB → Provider catalog** | ✅ done | `Agentic.LLM.Provider.CodingAgentBase` macro + thin per-agent wrappers (`Provider.Cursor`, `GeminiCli`, `Goose`, `Copilot`, `Kimi`, `Qwen`) so all the ACP-discoverable CLIs surface in `Catalog` and the Model Pathways card. `Canonical` extended with their canonical_id mappings. `ProviderTaxonomy` recognizes the wider CLI set. Adding a new agent is now ~5 lines. |
| **Follow-up: subscription fee prompt** | ✅ done | `Worth.LLM.SubscriptionPrompt.pending/0` flags any auto-detected CLI provider with `:subscription_included` and no `monthly_fee` stored. Chat view surfaces a dismissible banner per provider with a typical-fee hint, "Set fee" jumps to settings, "×" stores a sticky dismiss preference per provider. Banner refreshes when settings are saved so it disappears immediately once the fee is entered. |
